...
if TYPE_CHECKING:
    ...
    from .consumermdib import EntityConsumerMdib
    from sdc11073.mdib.statecontainers import AbstractMetricStateContainer, AbstractStateProtocol
...
class ConsumerMdibMethods:
    ...
    def mk_proposed_state(self, descriptor_handle: str,
        ...
        new_state = self._mdib.data_model.mk_state_container(entity.descriptor)
        if copy_current_state:
            if new_state.is_multi_state:
                if handle is None:  # new state
                    return new_state
                old_state = entity.states.get(handle)
            else:
    ...
    def sync_context_states(self):
        ...
        with self._mdib.entities.lock:
            for entity in self._mdib.entities.objects:
                is_dirty = False
                if entity.descriptor.is_context_state:
                    for handle in entity.states:
                        if handle not in devices_context_state_handles:
                            del entity.states[handle]
                            is_dirty = True
                if is_dirty:
