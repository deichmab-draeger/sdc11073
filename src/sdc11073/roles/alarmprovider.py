...
class GenericAlarmProvider(providerbase.ProviderRole):
    ...
    def make_operation_instance(self,
        ...
        pm_names = self._mdib.data_model.pm_names
        op_target_handle = operation_descriptor_container.OperationTarget
        op_target_entity = self._mdib.entities.handle.get_one(op_target_handle)
        if pm_names.SetAlertStateOperationDescriptor == operation_descriptor_container.NODETYPE:
            if pm_names.AlertSignalDescriptor == op_target_entity.descriptor.NODETYPE and op_target_entity.descriptor.SignalDelegationSupported:
                # operation_descriptor_container is a SetAlertStateOperationDescriptor
                set_state_descriptor_container = cast(AbstractSetStateOperationDescriptorContainer,
                                                      operation_descriptor_container)
                modifiable_data = set_state_descriptor_container.ModifiableData
                if 'Presence' in modifiable_data \
                    ...
                    self._logger.debug('GenericAlarmProvider: added handler "self._setAlertState" for %s target=%s',
                                       operation_descriptor_container, op_target_entity.descriptor)
                    return operation
    ...
    def _set_alert_system_states_initial_values(self):
        ...
        entities = self._mdib.entities.NODETYPE.get(pm_names.AlertSystemState, [])
    ...
    def _set_alert_states_initial_values(self):
        ...
        pm_types = self._mdib.data_model.pm_types
        pm_names = self._mdib.data_model.pm_names
        for alert_condition_entity in self._mdib.entities.NODETYPE.get(pm_names.AlertConditionState, []):
            alert_condition_entity.state.ActivationState = pm_types.AlertActivation.ON
            alert_condition_entity.state.Presence = False
        for alert_condition_entity in self._mdib.entities.NODETYPE.get(pm_names.LimitAlertConditionState, []):
            alert_condition_entity.state.ActivationState = pm_types.AlertActivation.ON
            alert_condition_entity.state.Presence = False

        for alert_signal_entity in self._mdib.entities.NODETYPE.get(pm_names.AlertSignalState, []):
            if alert_signal_entity.descriptor.SignalDelegationSupported:
                alert_signal_entity.state.Location = pm_types.AlertSignalPrimaryLocation.REMOTE
                alert_signal_entity.state.ActivationState = pm_types.AlertActivation.OFF
                alert_signal_entity.state.Presence = pm_types.AlertSignalPresence.OFF
            else:
                alert_signal_entity.state.ActivationState = pm_types.AlertActivation.ON
                alert_signal_entity.state.Presence = pm_types.AlertSignalPresence.OFF
    ...
    def _update_alert_system_states(mdib: ProviderMdib,
        ...
        def _get_alert_state(descriptor_handle: str) -> AbstractStateProtocol:
            ...
            if alert_state is None:
                # it is not part of this transaction
                alert_entity = mdib.entities.handle.get_one(descriptor_handle, allow_none=True)
                if alert_entity.state is None:
                    raise ValueError(f'there is no alert state for {descriptor_handle}')
                alert_state = alert_entity.state
            return alert_state

        for state in alert_system_states:
            all_child_entities = mdib.entities.parent_handle.get(state.DescriptorHandle, [])
            all_child_descriptors =  [e.descriptor for e in all_child_entities]
            all_child_descriptors.extend(
                [i.new for i in transaction.descriptor_updates.values() if
                 i.new.parent_handle == state.DescriptorHandle])
            all_alert_condition_descr = [d for d in all_child_descriptors if hasattr(d, 'Kind')]
    ...
    def _update_alert_signals(changed_alert_condition: AbstractStateProtocol,
        ...
        """
        pm_types = mdib.data_model.pm_types
        alert_signal_entities = mdib.entities.condition_signaled.get(changed_alert_condition.DescriptorHandle, [])
        # separate remote from local
        # remote_alert_signal_descriptors = [a.descriptor for a in alert_signal_entities if a.descriptor.SignalDelegationSupported]
        # local_alert_signal_descriptors = [a.descriptor for a in alert_signal_entities if not a.descriptor.SignalDelegationSupported]
        remote_alert_signal_entities = [a for a in alert_signal_entities if a.descriptor.SignalDelegationSupported]
        local_alert_signal_entities = [a for a in alert_signal_entities if not a.descriptor.SignalDelegationSupported]

        # look for active delegations (we only need the Manifestation value here)
        active_delegate_manifestations = []
        for remote_alert_signal_entity in remote_alert_signal_entities:
            alert_signal_state = remote_alert_signal_entity.state
            if alert_signal_state.Presence != pm_types.AlertSignalPresence.OFF and alert_signal_state.Location == 'Rem':
                active_delegate_manifestations.append(remote_alert_signal_entity.descriptor.Manifestation)
        ...
        for local_alert_signal_entity in local_alert_signal_entities:
            tr_item = transaction.get_state_transaction_item(local_alert_signal_entity.descriptor.Handle)
            if tr_item is None:
                is_delegated = local_alert_signal_entity.descriptor.Manifestation in active_delegate_manifestations  # is this local signal delegated?
                activation, presence = lookup[(changed_alert_condition.Presence, is_delegated)]
                alert_signal_state = transaction.get_state(local_alert_signal_entity.descriptor.Handle)
    ...
    def _get_fallback_signals(self,
        ...
        """
        if all_signal_descriptors is None:
            all_signal_entities = self._mdib.entities.condition_signaled.get(
                delegable_signal_descriptor.ConditionSignaled, [])
        return [tmp for tmp in all_signal_descriptors if not tmp.SignalDelegationSupported
                and tmp.Manifestation == delegable_signal_descriptor.Manifestation
                and tmp.ConditionSignaled == delegable_signal_descriptor.ConditionSignaled]
    ...
    def _delegate_alert_signal(self, params: ExecuteParameters) -> ExecuteResult:
        ...
        with self._mdib.alert_state_transaction() as mgr:
            ...
            for name in params.operation_instance.descriptor_container.ModifiableData:
                tmp = getattr(value, name)
                setattr(state, name, tmp)
            entity = self._mdib.entities.handle.get_one(operation_target_handle)
            if entity.descriptor.SignalDelegationSupported:
                if value.ActivationState == pm_types.AlertActivation.ON:
                    self._pause_fallback_alert_signals(entity.descriptor, None, mgr)
                else:
                    self._activate_fallback_alert_signals(entity.descriptor, None, mgr)
        return ExecuteResult(operation_target_handle,
                             self._mdib.data_model.msg_types.InvocationState.FINISHED)
    ...
    def _get_alert_system_states_needing_update(self) -> list[AbstractStateProtocol]:
        ...
        states_needing_update = []
        try:
            all_alert_system_ent = self._mdib.entities.NODETYPE.get(pm_names.AlertSystemDescriptor,
                                                                           [])
            for entity in all_alert_system_ent:
                # alert_system_state = self._mdib.states.descriptor_handle.get_one(alert_system_descr.Handle,
                #                                                                  allow_none=True)
                if entity.state is not None:
                    self_check_period = entity.descriptor.SelfCheckPeriod
                    if self_check_period is not None:
                        last_self_check = entity.state.LastSelfCheck or 0.0
                        if time.time() - last_self_check >= self_check_period - self.self_check_safety_margin:
                            states_needing_update.append(entity.state)
        except Exception:
            self._logger.error('_get_alert_system_states_needing_update: %r', traceback.format_exc())
        return states_needing_update
