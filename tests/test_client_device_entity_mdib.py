...
from sdc11073.entitymdib.consumermdib import EntityConsumerMdib
...
def runtest_realtime_samples(unit_test, sdc_device, sdc_client):
    ...
    for d_handle in d_handles:
        ...
        entity = client_mdib.entities.handle.get_one(d_handle)
        unit_test.assertEqual(entity.state.ActivationState, pm_types.ComponentActivation.ON)
        unit_test.assertIsNotNone(entity.state.MetricValue)
        unit_test.assertAlmostEqual(entity.state.MetricValue.DeterminationTime, time.time(), delta=0.5)
        unit_test.assertGreater(len(entity.state.MetricValue.Samples), 1)
    ...
    time.sleep(0.5)
    entity = client_mdib.entities.handle.get_one(d_handle)
    unit_test.assertEqual(entity.state.ActivationState, pm_types.ComponentActivation.OFF)
    unit_test.assertTrue(entity.state.MetricValue is None)
...
def runtest_metric_reports(unit_test, sdc_device, sdc_client, logger, test_periodic_reports=True):
    ...
    cl_mdib = EntityConsumerMdib(sdc_client)
    cl_mdib.init_mdib()
    ...
    unit_test.assertEqual(cl_entity1.state.MetricValue.MetricQuality.Validity, pm_types.MeasurementValidity.VALID)
    unit_test.assertEqual(cl_entity1.state.StateVersion, 1)  # this is the first state update after init
    unit_test.assertEqual(cl_entity1.state.PhysicalConnector, my_physical_connector)
...
class Test_Client_SomeDevice(unittest.TestCase):
    ...
    def test_alert_reports(self):
        ...
        for _activation_state, _actual_priority, _presence in product(list(pm_types.AlertActivation),
            ...
            coll.result(timeout=NOTIFICATION_TIMEOUT)
            entity = client_mdib.entities.handle.get_one( descriptor_handle)  # this shall be updated by notification
    ...
    def test_alert_condition_modification(self):
        ...
        entity = client_mdib.entities.handle.get_one(alert_descriptor_handle)
        self.assertEqual(entity.descriptor.SafetyClassification, pm_types.SafetyClassification.MED_C)

        entity = client_mdib.entities.handle.get_one(limit_alert_descriptor_handle)
        self.assertEqual(entity.descriptor.SafetyClassification, pm_types.SafetyClassification.MED_B)
        self.assertEqual(entity.descriptor.AutoLimitSupported, True)
        ...
        entity = client_mdib.entities.handle.get_one(alert_descriptor_handle)
        self.assertEqual(entity.state.ActualPriority, pm_types.AlertConditionPriority.HIGH)
        self.assertEqual(entity.state.Presence, True)

        # verify that alert system state is also updated
        parent_entity = client_mdib.entities.handle.get_one(entity.descriptor.parent_handle)
        self.assertTrue(alert_descriptor_handle in parent_entity.state.PresentPhysiologicalAlarmConditions)
        self.assertGreater(parent_entity.state.SelfCheckCount, 0)

        entity = client_mdib.entities.handle.get_one(limit_alert_descriptor_handle)
        self.assertEqual(entity.state.ActualPriority, pm_types.AlertConditionPriority.MEDIUM)
        self.assertEqual(entity.state.Limits, pm_types.Range(upper=Decimal(3)))
        self.assertEqual(entity.state.Presence, False)
        self.assertEqual(entity.state.MonitoredAlertLimits,
    ...
    def test_metadata_modification(self):
        ...
        cl_mds_entities = client_mdib.entities.NODETYPE.get(pm.MdsDescriptor)
        for cl_mds_entity in cl_mds_entities:
            self.assertEqual(cl_mds_entity.descriptor.MetaData.ModelNumber, '1.09')
            self.assertEqual(cl_mds_entity.descriptor.MetaData.Manufacturer[-1].text, 'My Company')
